// ============================================================
// GET /api/neti-courses
// 중앙교육연수원 전체 연수 목록 API
//
...
import {
  fetchNetiCourses,
  normalizeNetiCourses,
} from "@/lib/neti-courses";
import { checkRateLimit } from "@/lib/rate-limit";

const CACHE_KEY = "neti-courses-v2";
...
export async function GET(req) {
  ...
  if (existing) {
    console.log("[neti] In-Flight 대기 중...");
    try {
      ...
      return NextResponse.json(
        { error: "neti 목록 조회 실패 (in-flight 대기 중 에러)", detail: err.message },
        { status: 500 }
      );
    }
  }
  ...
  try {
    const { allRaw, externalRequestCount } = await fetchNetiCourses();

    const courses = normalizeNetiCourses(allRaw);
    ...
    console.log(
      `[neti] 완료: ${courses.length}개 / ${externalRequestCount}회 외부요청`
    );
    ...
    rejectInFlight(err);
    console.error("[neti] 수집 실패:", err.message);
    return NextResponse.json(
      { error: "중앙교육연수원 연수 목록을 가져오는 중 오류가 발생했습니다.", detail: err.message },
      { status: 500 }
    );
  } finally {
